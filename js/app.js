(function () {
  ...
  function init() {
    ...
    Worksheet.make('wsBox', {
      name: 'media-literacy',
      fields: [
        { id: 'q1', label: '① 見かけた情報', hint: 'そのまま書き写す。言い切りの表現に線を引くつもりで。', rows: 3,
          ph: '例：「◯◯を食べると必ずやせる」という投稿を見た' },
        { id: 'q2', label: '② 発信者はだれか', hint: '個人か組織か。専門家か。利害関係はあるか。', rows: 2,
          ph: '例：匿名アカウント。プロフィールにサプリの販売リンクがある' },
        { id: 'q3', label: '③ いつの情報か', hint: '古い情報が新しい話として回っていないか。', rows: 2, ph: '例：投稿は今週だが、引用元の記事は5年前' },
        { id: 'q4', label: '④ 根拠は示されているか', hint: 'データ・出典・調査方法。「みんな言っている」は根拠ではない。', rows: 3,
          ph: '例：グラフはあるが、何人を調べたのか書かれていない' },
        { id: 'q5', label: '⑤ 別の情報源で確かめた結果', hint: '公的機関・一次情報にあたる。', rows: 3,
          ph: '例：厚生労働省のページには、そのような効果は書かれていなかった' },
        { id: 'q6', label: '⑥ 判断と、そう考えた理由', hint: '「正しい／あやしい／判断できない」のどれかと、その理由。', rows: 2,
          ph: '例：あやしい。発信者に利害があり、根拠のデータが示されていないから' }
      ],
      build: function (v, e) {
        return '<h4>ファクトチェックシート</h4><dl>' +
          '<dt>① 見かけた情報</dt><dd>' + e(v.q1) + '</dd>' +
          '<dt>② 発信者</dt><dd>' + e(v.q2) + '</dd>' +
          '<dt>③ 情報の時期</dt><dd>' + e(v.q3) + '</dd>' +
          '<dt>④ 示されている根拠</dt><dd>' + e(v.q4) + '</dd>' +
          '<dt>⑤ 別の情報源で確かめた結果</dt><dd>' + e(v.q5) + '</dd>' +
          '<dt>⑥ 判断と理由</dt><dd>' + e(v.q6) + '</dd></dl>';
      },
      note: '「判断できない」も立派な結論です。分からないまま拡散しないことがメディアリテラシーです。'
    });
